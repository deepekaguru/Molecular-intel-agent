...
data_security = DataSecurityHandler()
...
from utils.security_guardrails import (
    validate_input,
    scrub_pii,
    validate_output,
    apply_clinical_guardrail,
    enforce_hitl,
    validate_patient_id,
    log_action,
    SecurityGuardrails
...
<div style='text-align:center; padding: 20px 0 30px;'>
    ...
    <div style='color:#7c3aed; font-weight:800; font-size:3rem; letter-spacing:3px; margin-bottom:8px;'>MOLECULAR INTEL</div>
    <div style='color:#9b7fd4; font-size:1.2rem; letter-spacing:4px; font-weight:300;'>TREATMENT AI</div>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    patient_id_raw = st.text_input("Patient ID", value="P-1001", max_chars=50)
    is_valid_id, id_error = validate_patient_id(patient_id_raw)
    if not is_valid_id:
        st.error(f"❌ {id_error}")
        patient_id = "P-INVALID"
    else:
    ...
    additional_notes = st.text_area(
        ...
        placeholder="Any other clinical details...",
        height=100
    )
...
if "validated_inputs" not in st.session_state:
...
if st.sidebar.button("🔬 Run Analysis"):
    ...
    valid_pid, pid_error = validator.validate_patient_id(patient_id)
    if not valid_pid:
        st.error(f"❌ {pid_error}")
        st.stop()

    valid_age, age_error = validator.validate_age(age)
    if not valid_age:
        st.error(f"❌ {age_error}")
        st.stop()

    if not mutations_selected:
        st.warning("⚠️ Please select at least one gene mutation to run the analysis.")
        st.stop()

    st.session_state["validated_inputs"] = {
        "patient_id": patient_id,
        "age": age,
        "gender": gender,
        "cancer_type": cancer_type,
        "prior_treatment": prior_treatment,
        "mutations_selected": mutations_selected,
...
if st.session_state.get("show_consent") and not st.session_state.get("consent_given"):
    ...
    with st.expander("📋 Understanding This Analysis", expanded=True):
        ...
        with col1:
            st.markdown("**✅ This System WILL:**")
            st.markdown("""
- Extract genomic features from clinical notes
- Search knowledge graph for treatment matches
- Rank treatments using ML algorithms
- Generate supporting clinical rationale
- Provide literature references
- Mask any detected PII in logs
            """)
        with col2:
            st.markdown("**❌ This System will NOT:**")
            st.markdown("""
- Prescribe specific dosages or schedules
- Make definitive diagnoses
- Replace physician clinical judgment
- Provide emergency medical advice
- Guarantee treatment outcomes
- Contact patients or healthcare providers
            """)

    st.info("**Important**: This system uses external AI services (OpenAI) which may incur costs and send data outside your system.")

    confirm_understanding = st.checkbox(
        "✓ I understand this is a research tool and will verify all recommendations with qualified healthcare professionals before any clinical use",
        key="confirm_research_tool"
    )

    if confirm_understanding:
        if st.button("▶️ Proceed with Analysis"):
...
if st.session_state.get("consent_given") and st.session_state.get("validated_inputs"):
    ...
    mutations_selected = inputs["mutations_selected"]
    additional_notes  = inputs["additional_notes"]

    data_security.secure_log("user_consent", {
        "patient_id": patient_id,
        "timestamp": datetime.now().isoformat(),
        "action": "analysis_authorized",
        "mutations_count": len(mutations_selected)
    })
    st.success("✓ User consent recorded - proceeding with analysis")
    st.markdown("---")

    is_suspicious, patterns = validator.detect_prompt_injection(additional_notes)
    if is_suspicious:
        st.error("⚠️ **Security Alert**: Suspicious input detected in clinical notes.")
        st.warning(f"Detected patterns: {', '.join(patterns[:3])}")
        data_security.secure_log('injection_attempt', {
            'patient_id': patient_id,
    ...
    clean_notes = validator.sanitize_clinical_notes(additional_notes)
    masked_result = data_security.mask_pii(clean_notes)
    if masked_result["pii_detected"]:
        st.warning(f"⚠️ **Privacy Protection**: Detected and masked: {', '.join(masked_result['pii_detected'])}")

    data_security.secure_log('analysis_started', {
        'patient_id': patient_id,
        'cancer_type': cancer_type,
        'mutation_count': len(mutations_selected),
        'has_notes': len(masked_result['masked_text']) > 0
    })

    safe_notes = masked_result['masked_text']
...
if 'state' in st.session_state:
    ...
    if not st.session_state.get("show_consent"):
        st.markdown("""
